...
export interface EventClientParameters<
...
export type EventActiveAssetCtxParameters = Omit<
  WsActiveAssetCtxRequest,
  "type"
>;
...
export type EventUserNonFundingLedgerUpdatesParameters = Omit<
  WsUserNonFundingLedgerUpdatesRequest,
  "type"
>;
...
> {
  ...
  activeAssetCtx(
    ...
    const channel = args.coin.startsWith("@")
      ? "activeSpotAssetCtx"
      : "activeAssetCtx";
  ...
  activeAssetData(
    ...
    return this.transport.subscribe(
      ...
      (event: CustomEvent<WsActiveAssetData>) => {
        if (
          event.detail.coin === args.coin &&
          event.detail.user === args.user.toLowerCase()
        ) {
          listener(event.detail);
        }
      },
