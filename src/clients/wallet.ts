...
export type ScheduleCancelParameters = Omit<
...
export type UpdateLeverageParameters = Omit<
...
export type CancelResponseSuccess = CancelResponse & {
  response: {
    data: {
      statuses: Exclude<
        CancelResponse["response"]["data"]["statuses"][number],
        { error: string }
      >[];
    };
  };
...
export type OrderResponseSuccess = OrderResponse & {
  response: {
    data: {
      statuses: Exclude<
        OrderResponse["response"]["data"]["statuses"][number],
        { error: string }
      >[];
    };
  };
...
export type TwapCancelResponseSuccess = TwapCancelResponse & {
  response: {
    data: {
      status: Exclude<
        TwapCancelResponse["response"]["data"]["status"],
        { error: string }
      >;
    };
  };
...
export type TwapOrderResponseSuccess = TwapOrderResponse & {
  response: {
    data: {
      status: Exclude<
        TwapOrderResponse["response"]["data"]["status"],
        { error: string }
      >;
    };
  };
  ...
  constructor(
    ...
    if (response.status === "err") {
      ...
      if ("statuses" in response.response.data) {
        ...
        const errors = response.response.data.statuses.reduce<string[]>(
          (acc, status, index) => {
        ...
        if (
          typeof response.response.data.status === "object" &&
          "error" in response.response.data.status
        ) {
...
> {
  ...
  constructor(args: WalletClientParameters<T, W>) {
    ...
    this.signatureChainId =
      args.signatureChainId ?? this._guessSignatureChainId;
  ...
  async approveAgent(
    ...
    const action: ApproveAgentRequest["action"] = {
      ...
      signatureChainId:
        typeof this.signatureChainId === "string"
          ? this.signatureChainId
              ...
              "limit" in modify.order.t
                ...
                : {
                    trigger: {
                      ...
                      triggerPx: this._formatDecimal(
                        modify.order.t.trigger.triggerPx
                      ),
  ...
  async claimRewards(signal?: AbortSignal): Promise<SuccessResponse> {
    // Construct an action
    const nonce = await this.nonceManager();
    const sortedAction: ClaimRewardsRequest["action"] = {
      type: "claimRewards",
    };
          ...
          "limit" in actionArgs.order.t
            ...
            : {
                trigger: {
                  ...
                  triggerPx: this._formatDecimal(
                    actionArgs.order.t.trigger.triggerPx
                  ),
  ...
  async registerReferrer(
    args: RegisterReferrerParameters,
    signal?: AbortSignal
  ): Promise<SuccessResponse> {
    // Construct an action
    const nonce = await this.nonceManager();
  ...
  async usdClassTransfer(
    args: UsdClassTransferParameters,
    signal?: AbortSignal
  ): Promise<SuccessResponse> {
    // Construct an action
  ...
  async vaultDistribute(
    args: VaultDistributeParameters,
    signal?: AbortSignal
  ): Promise<SuccessResponse> {
    // Construct an action
    const nonce = await this.nonceManager();
  ...
  protected async _guessSignatureChainId(): Promise<Hex> {
    ...
    if (isAbstractViemWalletClient(this.wallet)) {
      if (
        "getChainId" in this.wallet &&
        typeof this.wallet.getChainId === "function"
      ) {
        const chainId = (await this.wallet.getChainId()) as number;
        return `0x${chainId.toString(16)}`;
      }
    } else if (
      isAbstractEthersSigner(this.wallet) ||
      isAbstractEthersV5Signer(this.wallet)
    ) {
      if (
        "provider" in this.wallet &&
        typeof this.wallet.provider === "object" &&
        this.wallet.provider !== null &&
        "getNetwork" in this.wallet.provider &&
        typeof this.wallet.provider.getNetwork === "function"
      ) {
        const network = (await this.wallet.provider.getNetwork()) as {
  ...
  protected _validateResponse(
    ...
    if (response.status === "err") {
      ...
      if (
        response.response.data.statuses.some(
      ...
      response.response.type === "twapOrder" ||
      response.response.type === "twapCancel"
    ) {
      if (
        typeof response.response.data.status === "object" &&
        "error" in response.response.data.status
        ...
        throw new ApiRequestError(
          response as TwapOrderResponse | TwapCancelResponse
        );
      }
    }
  }
